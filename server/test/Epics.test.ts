...
test('전체 에픽 목록을 조회한다.', async () => {
  const res = await request(app).get('/api/epics?projectId=1');
  expect(res.status).toBe(200);
  expect(res.body).toEqual(mockData);
});

test('잘못된 projectId에 대해 응답코드 400을 반환한다.', async () => {
  const res = await request(app).get('/api/epics?projectId=9999');
  expect(res.status).toBe(400);
});

test('개별 에픽 정보 요청에 대한 응답을 반환한다.', async () => {
  const res = await request(app).get('/api/epics/1');
  expect(res.status).toBe(200);
...
test('잘못된 에픽 id로 보낸 요청에 대해 응답코드 404를 반환한다.', async () => {
  const res = await request(app).get('/api/epics/9999');
  expect(res.status).toBe(404);
});

test('에픽을 생성한다.', async () => {
  const res = await request(app).post('/api/epics').send({
    name: '새롭게 생성한 에픽',
    projectId: 1,
    startAt: new Date(),
    endAt: new Date(),
...
test('에픽을 수정한다.', async () => {
  ...
  // 어떻게 수정됐음을 확인할것인가?
  const edited = await request(app).get('/api/epics/5');
...
test('에픽을 삭제한다.', async () => {
  const res = await request(app).delete('/api/epics/5');
  expect(res.status).toBe(200);
});

test('잘못된 id로 에픽 삭제 요청을 보낼 시 응답코드 404을 반환한다.', async () => {
  const res = await request(app).delete('/api/epics/9999');
  expect(res.status).toBe(404);
});
