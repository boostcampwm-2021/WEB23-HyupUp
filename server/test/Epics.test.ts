...
test('전체 에픽 목록을 조회한다.', async () => {
  const req = request(app).get('/api/epics?projectId=1');
...
test('잘못된 projectId에 대해 응답코드 400을 반환한다.', async () => {
  const req = request(app).get('/api/epics?projectId=9999');
  req.set('Cookie', Cookies);
  const res = await req;
...
test('개별 에픽 정보 요청에 대한 응답을 반환한다.', async () => {
  const req = request(app).get('/api/epics/1');
  req.set('Cookie', Cookies);
  const res = await req;
  expect(res.status).toBe(200);
...
test('잘못된 에픽 id로 보낸 요청에 대해 응답코드 404를 반환한다.', async () => {
  const req = request(app).get('/api/epics/9999');
  req.set('Cookie', Cookies);
  const res = await req;
...
test('에픽을 생성한다.', async () => {
  const req = request(app).post('/api/epics').send({
    name: '새롭게 생성한 에픽',
    projectId: 1,
    startAt: new Date(),
    endAt: new Date(),
...
test('에픽을 수정한다.', async () => {
  ...
  // 어떻게 수정됐음을 확인할것인가?
  const edited = request(app).get('/api/epics/5');
...
test('잘못된 에픽 수정 요청에 대해 응답코드 400을 반환한다.', async () => {
  const req = request(app).patch('/api/epics/9999').send({
    name: '잘못 요청한 에픽',
  });
  req.set('Cookie', Cookies);
  const res = await req;
...
test('에픽을 삭제한다.', async () => {
  const req = request(app).delete('/api/epics/5');
  req.set('Cookie', Cookies);
  const res = await req;
  expect(res.status).toBe(200);
});
...
test('잘못된 id로 에픽 삭제 요청을 보낼 시 응답코드 404을 반환한다.', async () => {
  const req = request(app).delete('/api/epics/9999');
  req.set('Cookie', Cookies);
  const res = await req;
  expect(res.status).toBe(404);
});
