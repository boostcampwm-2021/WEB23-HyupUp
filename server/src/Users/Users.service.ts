...
export const getAllTasks = async (id: number, offset: number) => {
  ...
  SELECT ID as id, NAME as name, createdAt, updatedAt, USER_ID as userId, STATUS as status, PROJECT_ID AS projectId FROM TASKS WHERE USER_ID=${id} UNION SELECT ID, NAME, createdAt, updatedAt, USER_ID, STATUS, NULL AS PROJECT_ID FROM TODO WHERE USER_ID=${id} ORDER BY updatedAt DESC LIMIT ${limit} OFFSET ${offset};`);
  return tasks;
};
